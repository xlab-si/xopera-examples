...
Gets image from container, resizes it and stores it into destination container. 
...
def create_thumbnails(blob, container):
    ...
    azure_client = Azure()
    azure_client.create_client(azure_connection_string=conn_str)
    # retrieve original image from container
    original_image_path = "/tmp/" + str(blob)
    azure_client.retrieve_from_bucket(container, blob)
    ...
    for size_px in THUMBNAIL_SIZES_PX:
        ...
        azure_client.store_to_bucket(container_out_name, dest_file_name, resized_image_path)
